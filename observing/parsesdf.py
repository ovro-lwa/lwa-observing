...
def make_sched(sdf_fn, mode='buffer'):
    ...
    if session.obs_type in [ObsType.volt, ObsType.voltraw]:
...
def make_obs_list(inp:dict):
    ...
    if obs_type == ObsType.power.value or obs_type == ObsType.volt.value or obs_type == ObsType.voltraw.value:
        beam_num = int(inp['SESSION']['SESSION_DRX_BEAM'])
        do_cal = eval(inp['SESSION'].get('DO_CAL', 'False'))
        cal_dir = inp['SESSION'].get('CAL_DIR', None)
    elif obs_type != ObsType.power.value or obs_type != ObsType.volt.value or obs_type != ObsType.voltraw.value:
        beam_num = None
        do_cal = False
        cal_dir = None
    ...
    for i in inp['OBSERVATIONS']:
        ...
        if session.obs_type == ObsType.power or session.obs_type == ObsType.volt or session.obs_type == ObsType.voltraw:
...
#    ts += configure_buffer/(24*3600)
    for obs in obs_list:
        ...
        if mode == 'buffer':
            ...
            t0 = "'now'"
        cmd = (
            f"from observing import recmetadata as rm; _rec = con.start_dr(recorders=['dr'+str({session.beam_num})], "
            f"duration = {obs.obs_dur}, time_avg={obs.int_time}, t0 = {t0}); "
            f"rm.write_sidecars(_rec, '{session_mode_name}', {obs.obs_id})"
        )
        d.update({ts:cmd})
...
def volt_beam_obs(obs_list, session, mode='buffer'):
    ...
    for obs in obs_list:
        ...
        cmd = (
            f"from observing import recmetadata as rm; _rec = con.start_dr("
            f"recorders=['drt'+str({session.beam_num}{command_suffix})], duration = {obs.obs_dur}, time_avg=0, "
            f"t0={t0}, teng_f1={obs.freq1}*(196e6/2**32), teng_f2={obs.freq2}*(196e6/2**32), f0={obs.bw}, "
            f"gain1={beam_gain1}, gain2={beam_gain2}); "
            f"rm.write_sidecars(_rec, '{session_mode_name}', {obs.obs_id})"
        )
        d.update({ts:cmd})
