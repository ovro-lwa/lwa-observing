...
def volt_beam_obs(obs_list, session, mode='buffer'):
    ...
    for obs in obs_list:
        ...
        cmd = f"con.start_dr(recorders=['drt'+str({session.beam_num})], duration = {obs.obs_dur}, time_avg=0, t0={t0}, teng_f1={obs.freq1}*(196e6/2**32), teng_f2={obs.freq2}*(196e6/2**32), f0={obs.bw}, gain={beam_gain})"
        d.update({ts:cmd})
